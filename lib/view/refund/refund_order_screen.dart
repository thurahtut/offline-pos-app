...
class _RefundOrderScreenState extends State<RefundOrderScreen> {
  ...
  Widget _currentOrderListWidget() {
    ...
    return RawKeyboardListener(
      focusNode: _textNode,
      onKey: (event) {
        context.read<RefundOrderController>().handleKeyEvent(event);
      },
    ...
    List<Widget> calculatorActionWidgetList = [
      ...
      CommonUtils.eachCalculateButtonWidget(
        ...
        onPressed: () {
          ...
          for (Product product
              in context.read<RefundOrderController>().currentOrderList) {
            if ((product.refundQuantity ?? 0) > 0) {
              Product pp = Product.fromJson(
              ...
              pp.refundQuantity = 0;
              pp.lineId = pp.lineId;
              pp.refundedOrderLineId = pp.odooOrderLineId;
              pp.odooOrderLineId = null;
          ...
          if (context
              ...
              .selectedOrderList
              .isNotEmpty) {
            Navigator.pushNamed(context, SelectedRefundOrderScreen.routeName);
          } else {
            CommonUtils.showSnackBar(message: 'There is no items to refund!');
          }
        },
      ),
      CommonUtils.eachCalculateButtonWidget(
